...
- **Apache Site Manager** (`a2sitemgr`): Create and configure Apache virtual hosts with automatic SSL (automatically calls `fqdnmgr` if certbot is inistalled)
...
some of this command arguments are meant to be called from a2sitemgr command
examples:   
```bash
fqdnmgr purchase namecheap example.com # can be called by anyone
fqdnmgr certify namecheap #the domain is passed from a2sitemgr as an env var
fqdnmgr cleanup namecheap #the domain is passed from a2sitemgr as an env var
```
use `fqdnmgr --help` for detailed help
#### Apache Site Manager (`a2sitemgr`)
```bash
a2sitemgr -d example.com #creates apache2 config and home directory
a2sitemgr -swc -d mail.* -pp -p 8080 #subdomain wildcard, generates config with certs from all existing domains
a2sitemgr -pp -d something.example.com -p 3000 #proxy pass mode
```
