...
if ! grep -q "200 vpn_bypass" /etc/iproute2/rt_tables; then
    echo "200       vpn_bypass" | tee -a /etc/iproute2/rt_tables
fi
...
ip rule del to ${YOUR_PUBLIC_IP} iif ${YOUR_INTERFACE} lookup vpn_bypass 2>/dev/null
ip rule del from ${YOUR_PUBLIC_IP} oif ${YOUR_INTERFACE} lookup vpn_bypass 2>/dev/null
ip rule del from ${YOUR_PUBLIC_IP} lookup vpn_bypass priority 100 2>/dev/null
ip route flush table vpn_bypass 2>/dev/null
...
ip route replace ${YOUR_LAN_SUBNET} dev ${YOUR_INTERFACE} table vpn_bypass
ip route replace default via ${YOUR_DEFAULT_GATEWAY} dev ${YOUR_INTERFACE} table vpn_bypass
...
ip rule add from ${YOUR_PUBLIC_IP} lookup vpn_bypass priority 100
# ------------------------------